...
const CurrentPhase: React.FC<CurrentPhaseProps> = ({
  ...
  useEffect(() => {
    if (currentPhase?.prompts) {
      const combinedPrompts = currentPhase.prompts
        ...
        .map((prompt: Prompt) => injectValuesIntoPrompt(prompt.text!, answers))
        .join("\n");
      ...
      const combinedAiInstructions = currentPhase.prompts
        .filter(
          (prompt: Prompt) => prompt.type === "aiInstructions" && prompt.text
        )
        .map((prompt: Prompt) => injectValuesIntoPrompt(prompt.text!, answers))
        .join("\n");
          ...
          if (element.type === "checkbox") {
            ...
            if (Array.isArray(element.defaultValue)) {
              ...
              element.defaultValue.forEach((value) => {
                ...
                const choiceText =
                  typeof choice === "string" ? choice : choice?.text || value;
            ...
            const choice = element.choices?.find((c) =>
              typeof c === "string"
                ? c === element.defaultValue
                : c.value === element.defaultValue
            );
            const mappedValue =
              typeof choice === "string"
                ? choice
                : choice?.text || element.defaultValue;
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (formAction === "submit") {
      ...
      AIResponse = await sendPrompts(
        currentPhase?.prompts || null,
        answers,
        appId,
        surveyJson,
        currentPhaseIndex,
    ...
    <div>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {surveyJson?.phases?.length !== completedPhases.length &&
          currentPhase.elements &&
          currentPhase.elements.length > 0 &&
          (promptLoading ? (
