...
import { AIResponseDisplay, RunScoreDisplay } from "@/utils/phaseResultDisplay";
 ...
 * @returns
 */
const CompletedPhase: React.FC<CompletedPhaseProps> = ({ pageIndex, page }) => {
  const {
    answers,
    handleInputChange,
    setInputValue,
  ...
  useEffect(() => {
    if (surveyJson?.phases) {
      ...
      const allElements = surveyJson.phases.flatMap(
