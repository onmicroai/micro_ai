...
export default function FormBuilder() {
  ...
  const handleDragEnd = (result: any) => {
    ...
    if (sourceType === "palette") {
      const existingFieldsCount = phases.reduce(
        (count: number, phase: PhaseType) => {
          const elementCount =
            phase.elements?.filter((f) => f.type === draggableId).length ?? 0;
          const promptCount =
            phase.prompts?.filter((f) => f.type === draggableId).length ?? 0;
    ...
    if (source.droppableId === destination.droppableId) {
      const updatedPhases = phases.map((phase: PhaseType) => {
        if (Number(phase.id) === Number(sourcePhaseId)) {
          const items =
            sourceType === "fields" ? [...phase.elements] : [...phase.prompts];
   ...
   * @param instructions - Optional array of AI response instructions
   * @param isPrompt - Whether the field is a prompt (true) or an element (false)
   *
  ...
  const updateFieldText = (
    ...
    setPhases(
      phases.map((phase: PhaseType) => {
        ...
        const fieldExists = phase[isPrompt ? "prompts" : "elements"].some(
          (field: Element) => field.id === fieldId
        );

        if (fieldExists) {
          return {
            ...phase,
            [isPrompt ? "prompts" : "elements"]: phase[
              isPrompt ? "prompts" : "elements"
            ].map((field: Element) =>
              field.id === fieldId
                ? {
                    ...field,
   ...
   */
  const updateChatbotInstructions = (
    phaseId: string,
    fieldId: string,
    instructions: string
  ) => {
      ...
      phases.map((phase: PhaseType) => {
        if (phase.id === phaseId) {
          return {
            ...
            elements: phase.elements.map((field: Element) =>
              field.id === fieldId
                ? { ...field, chatbotInstructions: instructions }
                : field
            ),
          };
        }
      ...
      <div className="container mx-auto py-8 px-4 max-w-7xl">
              ...
              <Tooltip>
                ...
                <TooltipContent side="right" sideOffset={5}>
                  <p className="w-[200px] text-sm">
                    Provide a name and a description for your app that will be
                    displayed to users.
                  </p>
                </TooltipContent>
              </Tooltip>
        ...
        <div className="space-y-6 mb-8">
          ...
          <textarea
            value={description}
            onFocus={() => {
              if (description === "Tell the user what your app does...")
                setDescription("");
            }}
          ...
          <Collapsible>
            <CollapsibleTrigger asChild>
              <Button
                variant="ghost"
                className="w-full flex items-center justify-between p-2 -mt-2"
              >
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <TooltipProvider delayDuration={0}>
                    <Tooltip>
                      ...
                      <TooltipContent side="right" sideOffset={5}>
                        <p className="text-sm">
                          Optional settings to customize the privacy settings
                          and AI behavior in your app.
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <TooltipProvider delayDuration={0}>
                    <Tooltip>
                      ...
                      <TooltipContent side="right" sideOffset={5}>
                        <p className="w-[200px] text-sm">
                          Upload files that will be available to users of your
                          app. Supports PDF, PPT, DOC, TXT, CSV, JSON, and MD
                          files.
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        step="0.01"
                        min={
                          availableModels[aiConfig.aiModel || "gpt-4o-mini"]
                            ?.min ?? 0
                        }
                        max={
                          availableModels[aiConfig.aiModel || "gpt-4o-mini"]
                ...
                >
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <TooltipProvider delayDuration={0}>
                      <Tooltip>
                        ...
                        <TooltipContent side="right" sideOffset={5}>
                          <p className="w-[200px] text-sm">
                            A phase is a &ldquo;turn&rdquo; in the conversation
                            with AI, where the user provides inputs and the AI
                            provides a response. Give your phase a title and
                  ...
                  <div className="space-y-6">
                    ...
                    <Phase
                      ...
                      onUpdatePromptText={(fieldId, text, instructions) =>
                        updateFieldText(fieldId, text, instructions, true)
                      }
